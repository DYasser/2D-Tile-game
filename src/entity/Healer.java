...
public class Healer extends Entity{
	...
	private int prompt, 	count = -1, 	choice = 0, 	r;
	
	public Healer(Camera cam, Sprite sprite, VectorXY origin, String[] msg, int size) {
			...
			if(cam.getBoundsOnScreen().collides(this.bounds)) {	//inside camera
				if(sense.colCircleBox(player.getBounds()))	//player inside sense
				{
					...
					if(display)			//display msg
					{
						...
						Entity.stop = true; // pause all other entities
					}
					else
						Entity.stop = false;
					
					if(player.getInteract() && count < prompt)	//player still reading dialogue
					{
						pressedBefore = true; 
					}
					else if(!player.getInteract() && count < prompt && pressedBefore) {	//pressed
						count++;
						display = true;
						pressedBefore = false;  
					}
					else if(player.getInteract() && count == prompt)	//end of dialogue
					{
						interact = false;
						display = false;
	...
	public void render(Graphics2D g) {
		...
		if(cam.getBoundsOnScreen().collides(this.bounds)) {	//on screen
			/*
			//show all bounds 
		...
		if(display)	//inside dialogue
		{
			...
			if(count < prompt)	//still in dialogue
