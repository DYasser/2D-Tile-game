...
public class Traitor extends Entity{
	...
	public Traitor(Camera cam, Sprite sprite, VectorXY orgin, String[] msg, int size) {
		...
		acc = 0;	//acceleration and maximum speed
		maxSpeed = 0; 
		
		r = 150; 
		sense = new AABB(new VectorXY(orgin.x + size/2 - r/2, orgin.y + size/2 - r/2 +10), r, this);
	...
	public void update()
	{
		for(int i = 0; i < Player.players.size(); i++)	//going through all players
		{
			...
			if(cam.getBoundsOnScreen().collides(this.bounds) && !fallen)  {//inside camera and not skeleton
				
				if(sense.colCircleBox(player.getBounds()))	//player inside range
				{
					interact = true; 
					if(display)	//show dialogue
					{
						if(player.up)	//presses up
						{
							choice = 1;
						}
						if(player.down)	//presses up
						{
							choice = 0;
						}
						Entity.stop = true;
					...
					{ 
						...
						{ 
							...
							new Skeleton(cam, new Sprite("entity/skeleton.png",32,32), pos, 64);	//changes to enemy
							fallen = true;
						}
					}
				} 
	...
	public void render(Graphics2D g) {
		...
		if(cam.getBoundsOnScreen().collides(this.bounds) && !fallen) { // inside camera and not changed to skeleton
			...
			*/
			...
			if(interact) //show how to interact
			{
